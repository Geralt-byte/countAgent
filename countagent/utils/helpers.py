...
def strip_think(text: str) -> str:
    ...
    Covers:
      1. Well-formed `なん...` and `<thought>...</thought>` blocks.
    ...
    text = re.sub(r"なん[\s\S]*?</think\s*>", "", text)
...
_TOOL_RESULTS_DIR = ".countagent/tool-results"
...
def build_status_content(
    *,
    ...
    lines = [
        f"\U0001f408 countagent v{version}",
...
def sync_workspace_templates(workspace: Path, silent: bool = False) -> list[str]:
    ...
    try:
        tpl = pkg_files("countagent") / "templates"
    except Exception:
        return []
    if not tpl.is_dir():
        return []
    ...
    try:
        from countagent.utils.git_backend import GitStore
