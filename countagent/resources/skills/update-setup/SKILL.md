---
name: update-setup
description: One-time setup wizard for the countagent upgrade skill. Triggers: setup update, configure update, 切设置更新, 初始化更新.
---
...
Use `exec` to run `countagent --version`. Tell the user the current version.

Then collect install clues with `exec`. These commands are best-effort; if one fails,
keep going and show the useful output:

```
command -v countagent || true
python -m pip show countagent-ai || true
pipx list | sed -n '/countagent-ai/,+3p' || true
uv tool list | sed -n '/countagent-ai/,+3p' || true
```
...
answer, stop and ask the user to rerun this setup when they know how countagent was
installed.
...
`question: "Where is your countagent source checkout? Enter an absolute path or a path relative to this workspace:"`.
...
| Method | Command |
|--------|---------|
| uv | `uv tool install "countagent-ai[EXTRAS]" --force` |
| pipx | `pipx install --force "countagent-ai[EXTRAS]"` |
| pip | `python -m pip install --upgrade "countagent-ai[EXTRAS]"` |
...
---
name: update
description: "Upgrade countagent to the latest version. Triggers: upgrade countagent, update countagent, 升级countagent, 更新countagent."
---
...
4. Use `exec` to verify: `countagent --version`
5. Tell the user the new version. Say: "Run `/restart` to restart countagent and apply the update. If `/restart` is unavailable in this channel, restart the countagent process manually."
```

## Step 5: Confirm

Tell the user: "Upgrade skill created. Say 'upgrade countagent' when you want to update."
