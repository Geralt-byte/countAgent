...
# Remove default handler and re-add with unified countagent format
logger.remove()
...
from countagent import __logo__, __version__
...
from countagent.infra.cli.stream import StreamRenderer, ThinkingSpinner
from countagent.infra.config.paths import get_workspace_path, is_default_workspace
from countagent.infra.config.schema import Config
from countagent.utils.helpers import sync_workspace_templates
from countagent.utils.restart import (
...
app = typer.Typer(
    ...
    help=f"{__logo__} countagent - Personal AI Assistant",
...
def _init_prompt_session() -> None:
    ...
    from countagent.infra.config.paths import get_cli_history_path
...
def _print_agent_response(
    ...
    console.print()
    console.print(f"[cyan]{__logo__} countagent[/cyan]")
    console.print(body)
    console.print()
...
async def _print_interactive_response(
    ...
    def _write() -> None:
        ...
        ansi = _render_interactive_ansi(
            lambda c: (
                c.print(),
                c.print(f"[cyan]{__logo__} countagent[/cyan]"),
...
def version_callback(value: bool):
    if value:
        console.print(f"{__logo__} countagent v{__version__}")
        raise typer.Exit()
...
def main(
    ...
    """countagent - Personal AI Assistant."""
    pass
...
def onboard(
    ...
    """Initialize countagent configuration and workspace."""
    from countagent.infra.config.loader import get_config_path, load_config, save_config, set_config_path
    from countagent.infra.config.schema import Config
    ...
    if wizard:
        from countagent.infra.cli.onboard import run_onboard
        ...
        try:
            ...
            console.print("[yellow]Please run 'countagent onboard' again to complete setup.[/yellow]")
            raise typer.Exit(1)
    ...
    agent_cmd = 'countagent agent -m "Hello!"'
    gateway_cmd = "countagent gateway"
    if config:
        agent_cmd += f" --config {config_path}"
        gateway_cmd += f" --config {config_path}"

    console.print(f"\n{__logo__} countagent is ready!")
    console.print("\nNext steps:")
    if wizard:
    ...
    console.print(
        "\n[dim]Want Telegram/WhatsApp? See: https://github.com/HKUDS/countagent#-chat-apps[/dim]"
    )
...
def _onboard_plugins(config_path: Path) -> None:
    ...
    from countagent.channels.registry import discover_all
...
def _make_provider(config: Config):
    ...
    """
    from countagent.providers.factory import make_provider
...
def _load_runtime_config(config: str | None = None, workspace: str | None = None) -> Config:
    ...
    from countagent.infra.config.loader import load_config, resolve_config_env_vars, set_config_path
...
def _warn_deprecated_config_keys(config_path: Path | None) -> None:
    ...
    from countagent.infra.config.loader import get_config_path
...
def _migrate_cron_store(config: "Config") -> None:
    ...
    from countagent.infra.config.paths import get_cron_dir
...
def serve(
    ...
    verbose: bool = typer.Option(False, "--verbose", "-v", help="Show countagent runtime logs"),
    workspace: str | None = typer.Option(None, "--workspace", "-w", help="Workspace directory"),
    config: str | None = typer.Option(None, "--config", "-c", help="Path to config file"),
):
    ...
    try:
        ...
        console.print("[red]aiohttp is required. Install with: pip install 'countagent-ai[api]'[/red]")
        raise typer.Exit(1)

    from loguru import logger
    from countagent.engine.loop import AgentLoop
    from countagent.infra.api.server import create_app
    from countagent.core.bus import MessageBus
    from countagent.infra.sessions.manager import SessionManager

    if verbose:
        logger.enable("countagent")
    else:
        logger.disable("countagent")
...
def _run_gateway(
    ...
    *,
    ...
    from countagent.engine.loop import AgentLoop
    from countagent.tools.cron import CronTool
    from countagent.tools.message import MessageTool
    from countagent.core.bus import MessageBus
    from countagent.channels.manager import ChannelManager
    from countagent.infra.scheduling.cron_service import CronService
    from countagent.infra.scheduling.cron_types import CronJob
    from countagent.heartbeat.service import HeartbeatService
    from countagent.providers.factory import build_provider_snapshot, load_provider_snapshot
    from countagent.infra.sessions.manager import SessionManager

    port = port if port is not None else config.gateway.port

    console.print(f"{__logo__} Starting countagent gateway version {__version__} on port {port}...")
    ...
    from countagent.engine.loop import UNIFIED_SESSION_KEY
    ...
    async def on_cron_job(job: CronJob) -> str | None:
        ...
        from countagent.utils.evaluator import evaluate_response
    ...
    from countagent.infra.scheduling.cron_types import CronJob, CronPayload
...
def agent(
    ...
    logs: bool = typer.Option(False, "--logs/--no-logs", help="Show countagent runtime logs during chat"),
):
    ...
    if message:
        ...
        from countagent.core.bus import InboundMessage
...
def channels_status(
    ...
    """Show channel status."""
    from countagent.channels.registry import discover_all
    from countagent.infra.config.loader import load_config, set_config_path
...
def _get_bridge_dir() -> Path:
    ...
    pkg_bridge = Path(__file__).parent.parent / "bridge"  # countagent/bridge (installed)
    ...
    if not source:
        ...
        console.print("Try reinstalling: pip install --force-reinstall countagent")
        raise typer.Exit(1)
...
def status():
    ...
    console.print(f"{__logo__} countagent Status\n")

    console.print(f"Config: {config_path} {'[green]✓[/green]' if config_path.exists() else '[red]✗[/red]'}")
    console.print(f"Workspace: {workspace} {'[green]✓[/green]' if workspace.exists() else '[red]✗[/red]'}")

    if config_path.exists():
        from countagent.providers.registry import PROVIDERS
...
def _logout_github_copilot() -> None:
    ...
    try:
        from countagent.providers.github_copilot_provider import get_storage
    except ImportError:
...
def _login_github_copilot() -> None:
    try:
        from countagent.providers.github_copilot_provider import login_github_copilot
